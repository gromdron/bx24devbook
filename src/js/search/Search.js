...
export default class Search extends preact.Component {
  ...
  getResults() {
    ...
    if (this.state.search.length < settings.minimumLength) {
      ...
      warnings.push(
        settings.minimumLength === 1
          ? textSearchOneCharacterOrMore
          : textSearchShouldBeXOrMore.replace("!min", settings.minimumLength)
      );
    ...
    const found = Object.values(
      this.props.onSearch(this.state.search).reduce((acc, fieldResult) => {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="SearchResults" ref={el => (this.resultRef = el)}>
          <input
          ...
          />
          <button className="SearchResults__close" onClick={this.handleClose}>
            &times;
          </button>
          ...
          {results.map(result => (
            <Result key={result.title} item={result} settings={settings} />
          ))}

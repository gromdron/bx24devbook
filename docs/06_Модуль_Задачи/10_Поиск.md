...
2. Экспериментальный провайдер возвращающий доменные сущности `Bitrix\Tasks\V2\Public\Provider\TaskProvider`
...
> Перед началом изучения важно отметить что `TaskProvider` это специализированная надстройка над `TaskList`, поэтому если вы хотите получить максимальную гибкость и совсем незначительное количество производительности, то лучше сразу перейти к рассмотрению `TaskList` далее.

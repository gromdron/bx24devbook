...
Иногда бывает необходимость достать только те лиды, для которых есть запись в сторонней таблице. Для этого можно воспользоваться `__JOINS` ключем.
...
20. Добавление наблюдетей в канбане

...
Как видите, мы используем цикл по возвращаемым значениям из `$dialog->getMap()`, однако мы так же можем явно отрисовать наши параметры, используюя следующий примем:
